...
import BrandTable from "./DiaLGTableBrand";
...
export default function BrandDialoG() {
  ...
  const getColors = async () => {
    try {
      const res = await axios.get("http://localhost:8080/thuong-hieu/get-all2");
      if (res.status === 200) {
        setSizes(res.data);
        setSelected({
          id: -99,
  ...
  const deleteColors = async () => {
    try {
      const res = await axios.delete(
        "http://localhost:8080/thuong-hieu/delete/" + selected.id
      );
      if (res.status === 200) {
  ...
  const reCoverColors = async () => {
    try {
      const res = await axios.put(
        "http://localhost:8080/thuong-hieu/recover/" + selected.id,
        {
          ten: selected.ten,
          trangThai: true,
        }
    ...
    if (selected.id === -99) {
      try {
        const res = await axios.post(
          "http://localhost:8080/thuong-hieu/create",
          {
            ten: selected.ten,
            trangThai: true,
          }
      ...
      try {
        const res = await axios.put(
          "http://localhost:8080/thuong-hieu/update/" + selected.id,
          {
            ten: selected.ten,
            trangThai: true,
          }
  ...
  return (
    <React.Fragment>
      <Tooltip title="Danh sách Thương hiệu">
        <Button
      ...
      <Dialog open={open} onClose={handleClose}>
        <DialogTitle>Thương hiệu</DialogTitle>
        <DialogContent>
          ...
          <BrandTable rows={sizes} selected={setSelected} />
        </DialogContent>
      </Dialog>
    </React.Fragment>
  );
